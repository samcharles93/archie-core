...
organization access control. The [event automation design](docs/prds/event-automation.md)
and [organizations and access design](docs/prds/orgs-and-access.md) describe
those capabilities and their delivery stages.
...
- **Infrastructure operations:** A monitoring alert starts a diagnostic workflow
  to inspect a service, collect evidence, and propose or perform an authorized
  recovery action.
...
run with a scratch workspace, require a repository, or accept one optionally.
An investigation can finish with a report; a maintenance workflow can finish
with an operational result; a coding workflow can finish with a pull request.
...
See the [firewall investigation example](examples/workflows/firewall-investigate.yaml)
for a workflow that uses typed inputs, a network investigator profile, and no
repository.

## Organizations, identities, and controlled execution

The approved access model organizes automation into **organizations** and
**workspaces**. An organization owns its members, agent identities, secrets,
...
Each organization can enable the default workflows it needs and add its own.
...
through the organization, workspace, identity, workflow, and individual object.
Lower levels can narrow access, and a denial takes precedence. Organization and
workspace policy also determines which actions require approval from another
member.

Authorized runs receive credentials scoped to their identity, workspace, and
work. Event data supplies the task's inputs; it cannot grant additional access.
The [organizations and access design](docs/prds/orgs-and-access.md) defines this
...
example: identify its structure, map the fields you need, select a workflow,
and approve the binding.
...
workflows, modules, channels, and forges. The [playbook engine design](docs/prds/eda-playbook-engine.md)
describes how registered capabilities become actions that event rules can call.
...
- [Event automation](docs/prds/event-automation.md) — sources, schemas, mappings, bindings, workflows, and agent profiles.
- [Organizations and access](docs/prds/orgs-and-access.md) — tenancy, identities, permissions, approvals, and audit.
- [Playbook engine](docs/prds/eda-playbook-engine.md) — event rules and extensible actions.
