...
const {
  ...
  parseSchema,
  schemaAt,
} = await import("../src/settings/resource-schema.ts");
...
const objectSchema = {
  ...
  properties: {
    email: {
      type: "object",
      properties: { listen_addr: { type: "string", title: "Listen address" } },
    },
    rate_limit: {
      type: "object",
      properties: {
        ...
        max_requests: {
          type: "integer",
          description: "0 disables rate limiting.",
        },
      },
    },
  },
...
const arraySchema = {
  ...
  items: {
    type: "object",
    properties: { owner: { type: "string" }, base: { type: "string" } },
  },
};
const keyedSchema = {
  type: "object",
...
test("schemaAt walks nested fields, array items and keyed-collection values", () => {
  assert.deepEqual(
    schemaAt(
      objectSchema,
      "channel-settings.rate_limit.window",
      "channel-settings",
    ),
    {
      type: "string",
  ...
  assert.deepEqual(
    schemaAt(arraySchema, "repositories.0.owner", "repositories"),
    { type: "string" },
  );
  assert.deepEqual(
    schemaAt(keyedSchema, "providers.anything.class", "providers"),
    { type: "string" },
  );
  assert.deepEqual(
    schemaAt(objectSchema, "channel-settings", "channel-settings"),
    objectSchema,
  );
});
...
test("schemaAt answers nothing for a path the schema does not describe", () => {
  assert.equal(
    schemaAt(
      objectSchema,
      "channel-settings.telegram.token",
      "channel-settings",
    ),
    null,
  );
  assert.equal(
    schemaAt(null, "channel-settings.email", "channel-settings"),
    null,
  );
  assert.equal(
    schemaAt(
      bareSchema,
      "channel-settings.email.listen_addr",
      "channel-settings",
    ),
    null,
  );
...
test("fieldTitle prefers the schema title over the dashed-key fallback", () => {
  assert.equal(
    fieldTitle("listen_addr", { title: "Listen address" }),
    "Listen address",
  );
...
test("fieldPlaceholder offers a duration only where the schema says duration", () => {
  assert.equal(
    fieldPlaceholder({ type: "string", format: "duration" }),
    "1m0s",
  );
  assert.equal(
    fieldPlaceholder({ type: "string", format: "date-time" }),
...
test("fieldHint carries the schema's own prose, and nothing when it has none", () => {
  assert.equal(
    fieldHint({ description: "0 disables rate limiting." }),
...
test("blankFromSchema synthesises a new row from the schema, not from a path", () => {
  ...
  assert.deepEqual(
    blankFromSchema({ type: "array", items: { type: "string" } }),
    [],
  );
...
test("the live editor reads the schema, and no longer hardcodes document shapes", async () => {
  const editor = await readFile(
    new URL("../src/settings/StructuredValueEditor.vue", import.meta.url),
    "utf8",
  );
  assert.match(
    editor,
    /fieldPlaceholder/,
    "the editor offers the duration placeholder from the schema",
  );
  assert.match(editor, /fieldTitle/, "labels come from the schema title");
  assert.match(
    editor,
    /blankFromSchema/,
    "a new row is synthesised from the schema",
  );
  for (const stale of [
    'base: "main"',
    'ecosystem: "go"',
    ...
    assert.ok(
      !editor.includes(stale),
      `the editor still hardcodes a document shape: ${stale}`,
    );
  }
});
...
test("the resource card hands the derived schema to the editor", async () => {
  const card = await readFile(
    new URL("../src/settings/StructuredResourceCard.vue", import.meta.url),
    "utf8",
  );
