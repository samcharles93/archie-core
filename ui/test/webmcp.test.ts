...
const { registerWebMcpTools, createWebMcpRegistrar } = await import("../src/lib/webmcp.ts");
...
function fakeClient(): { calls: Call[]; client: Parameters<typeof archieWebMcpTools>[0] } {
  const calls: Call[] = [];
  ...
  return {
    ...
    client: {
      ...
      taskAction: async (id: string, action: string) => reply("taskAction", [id, action]),
    },
  };
}

function toolNamed(client: Parameters<typeof archieWebMcpTools>[0], name: string) {
  const tool = archieWebMcpTools(client).find((candidate) => candidate.name === name);
...
test("registration no-ops without document.modelContext instead of throwing", async () => {
  ...
  const registration = await registerWebMcpTools(archieWebMcpTools(fakeClient().client));
...
test("registration passes each tool's schema and annotations, and disposes via the signal", async () => {
  ...
  for (const tool of registered) {
    ...
    assert.ok(tool.signal instanceof AbortSignal, `${tool.name} has no abort signal`);
    assert.equal(tool.signal.aborted, false);
  }

  registration.dispose();
  for (const tool of registered) {
    assert.equal(tool.signal?.aborted, true, `${tool.name} was not unregistered`);
  }
});

test("read-only tools are read-only; recovery tools are consequential", () => {
  const annotations = new Map(
    archieWebMcpTools(fakeClient().client).map((tool) => [tool.name, tool.annotations]),
  );

  for (const name of ["list_tasks", "get_task", "recent_events", "daemon_health"]) {
    assert.equal(annotations.get(name)?.readOnlyHint, true, `${name} should be read-only`);
    assert.notEqual(annotations.get(name)?.consequentialHint, true, `${name} must not be consequential`);
  }
  for (const name of ["retry_task", "stop_task", "cancel_task"]) {
    assert.equal(annotations.get(name)?.consequentialHint, true, `${name} needs a confirmation`);
    assert.notEqual(annotations.get(name)?.readOnlyHint, true, `${name} must not be read-only`);
  }
});
...
test("tools returning agent, operator or inbound content carry untrustedContentHint", () => {
  ...
  for (const name of ["list_tasks", "get_task", "recent_events"]) {
    assert.equal(annotations.get(name)?.untrustedContentHint, true, `${name} returns untrusted content`);
  }
...
test("execute returns the envelope and calls the dashboard's own client methods", async () => {
  ...
  assert.deepEqual(JSON.parse(listed.content[0].text), { ok: true, method: "tasks", args: [] });
...
test("a recovery tool without a task id fails before touching the API", async () => {
  const { calls, client } = fakeClient();
  await assert.rejects(() => Promise.resolve(toolNamed(client, "retry_task").execute({})), /task_id is required/);
  assert.equal(calls.length, 0);
});

test("re-registering replaces the previous registration", async () => {
  const { registered } = installModelContext();
  const register = createWebMcpRegistrar(archieWebMcpTools(fakeClient().client));
  ...
  for (const tool of registered.slice(0, count)) {
    assert.equal(tool.signal?.aborted, true, "the first registration was left live");
  }
});
