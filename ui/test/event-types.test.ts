...
const { captureIdentity, cleanRule, parseHeaderLines, ruleSummary } =
  await import("../src/captures/event-types.ts");
...
test("a capture reads as its type's name, as unidentified, or as a type since deleted", () => {
  ...
  assert.deepEqual(captureIdentity({ id: 1, event_type: "et1" }, types), {
    label: "push",
    identified: true,
  });
  assert.deepEqual(captureIdentity({ id: 2, event_type: "" }, types), {
    label: "Unidentified",
    identified: false,
  });
  assert.deepEqual(captureIdentity({ id: 3 }, types), {
    label: "Unidentified",
    identified: false,
  });
  assert.deepEqual(captureIdentity({ id: 4, event_type: "gone" }, types), {
    label: "Deleted type",
    identified: true,
  });
});
