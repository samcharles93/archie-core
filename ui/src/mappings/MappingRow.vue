...
<template>
  <TableRow>
    ...
    <TableCell class="font-mono text-fg-muted">{{
      eventTypeLabel(mapping.event_type_id, eventTypes)
    }}</TableCell>
    ...
    <TableCell class="text-fg-muted">{{
      mapping.match_count ? ago(mapping.last_matched_at) : "—"
    }}</TableCell>
    <TableCell>
      <div class="flex justify-end gap-2">
        <Button variant="outline" size="xs" @click="emit('edit', mapping)"
          >Edit</Button
        >
        <Button variant="destructive" size="xs" @click="emit('delete', mapping)"
          >Delete</Button
        >
      </div>
    </TableCell>
