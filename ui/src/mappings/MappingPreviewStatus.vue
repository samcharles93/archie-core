 ...
 */
const props = defineProps<{ field: MappingField; preview: Preview | null; failure?: PreviewFailure }>();
...
const status = computed<{ variant: "ok" | "danger" | "idle"; text: string }>(() => {
  ...
  if (Object.prototype.hasOwnProperty.call(props.preview.values || {}, props.field.name)) {
    return { variant: "ok", text: JSON.stringify(props.preview.values?.[props.field.name]) };
  }
