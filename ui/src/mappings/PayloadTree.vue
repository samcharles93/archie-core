...
import { fieldTypeFromValue, pathAppendIndex, pathAppendKey, type FieldPick } from "./mapping-fields";
 ...
 */
const props = withDefaults(defineProps<{ value: unknown; path?: string; name?: string }>(), { path: "", name: "" });
...
const entries = computed<Array<{ key: string; value: unknown; path: string }>>(() => {
  ...
  return Object.entries(props.value as Record<string, unknown>).map(([key, child]) => ({
    key,
    value: child,
...
<template>
  <ul v-if="isBranch" class="flex list-none flex-col gap-0.5 pl-4 font-mono text-xs">
    <li v-for="entry in entries" :key="entry.path">
      ...
      <PayloadTree :value="entry.value" :path="entry.path" :name="entry.key" @pick="emit('pick', $event)" />
    </li>
  </ul>
