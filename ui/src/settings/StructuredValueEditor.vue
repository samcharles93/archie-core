...
const readOnly = computed(
  () =>
    props.locked ||
    /(^|\.)(has_credential|credential_configured|headers_configured)$/.test(
      props.path,
    ),
);
...
const property = computed(() =>
  schemaAt(props.schema, props.path, props.rootPath),
);
...
function addArray(): void {
  ...
  emit("update:modelValue", [
    ...current,
    blankFromSchema(property.value?.items),
  ]);
}
...
function removeArray(index: number): void {
  emit(
    "update:modelValue",
    (props.modelValue as unknown[]).filter(
...
function scalar(value: string | number): void {
  emit(
    "update:modelValue",
    typeof props.modelValue === "number" ? Number(value) : value,
  );
}
</script>
...
<template>
  <div v-if="isArray" class="space-y-2 rounded-lg border border-border/70 p-3">
    <div
      v-for="(item, index) in modelValue as unknown[]"
      :key="index"
    ...
    >
      ...
      <Button
        ...
        variant="ghost"
        size="icon-sm"
        :aria-label="`Remove ${label(path)} row ${index + 1}`"
        @click="removeArray(index)"
      >
        <Trash2 />
      </Button>
    </div>
    <Button type="button" variant="outline" size="sm" @click="addArray"
      ><Plus /> Add row</Button
    >
  </div>
  ...
  <fieldset
    v-else-if="isObject"
    class="space-y-4 rounded-lg border border-border/70 p-3"
  >
    <div v-for="[key, value] in entries" :key="key" class="space-y-1.5">
      <div class="flex items-center gap-2">
        <Label :for="`${path}.${key}`" class="flex-1">{{
          fieldTitle(key, propertyFor(key))
        }}</Label>
        <template
          v-if="
            path.endsWith('_map') ||
        ...
        >
          <Input
            ...
            @change="
              renameObject(key, ($event.target as HTMLInputElement).value)
            "
          />
          <Button
            type="button"
      ...
      <p v-if="fieldHint(propertyFor(key))" class="text-xs text-fg-subtle">
        {{ fieldHint(propertyFor(key)) }}
      </p>
      <StructuredValueEditor
        :id="`${path}.${key}`"
        :model-value="value"
  ...
  >
    ...
    <span v-if="readOnly" class="text-xs text-muted-foreground"
      >Reported by Archie</span
    >
  </div>
  <Textarea
    v-else-if="path.endsWith('.prompt')"
    :id="path"
  ...
  <div v-else class="space-y-1">
    <Input
      ...
      @update:model-value="scalar"
    />
    <p v-if="fieldHint(property)" class="text-xs text-fg-subtle">
      {{ fieldHint(property) }}
    </p>
  </div>
</template>
