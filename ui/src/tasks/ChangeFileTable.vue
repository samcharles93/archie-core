...
const total = computed(
  () => Number(props.capture.totals?.files) || files.value.length,
);
</script>
...
<template>
  <p v-if="!files.length" class="py-3 text-xs text-fg-muted">
    This capture recorded no file entries{{
      total
        ? `, though its totals cover ${total} file${total === 1 ? "" : "s"}`
        : ""
    }}.
  </p>
  <template v-else>
      ...
      <TableBody>
        <TableRow v-for="(file, i) in files" :key="`${file.path}:${i}`">
          <TableCell class="font-mono break-all whitespace-normal">
            <span v-if="file.old_path" class="text-fg-muted"
              >{{ file.old_path }} → </span
            >{{ file.path }}
          </TableCell>
          <TableCell>
            ...
            <span v-if="file.binary" class="block text-xs text-fg-muted"
              >no textual hunks</span
            >
          </TableCell>
