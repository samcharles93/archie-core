...
const name = computed(
  () => props.tool.name || props.tool.Name || props.tool.tool || "tool",
);
const parameters = computed(
  () => props.tool.parameters || props.tool.Parameters || "",
);
const failed = computed(() =>
  Boolean(props.tool.err || props.tool.Err || props.tool.failed),
);
...
<template>
  ...
  <div v-else class="flex items-baseline gap-1.5 [overflow-wrap:anywhere]">
    ...
    <code v-if="parameters" class="font-mono text-[0.95em] text-fg-subtle">{{
      parameters
    }}</code>
    <span :class="cn('text-fg-muted', failed && 'text-danger')">{{
      summary
    }}</span>
  </div>
</template>
