  ...
  <div class="flex items-center border-b px-3 py-1.5 text-xs">
    <Popover v-model:open="open">
      <PopoverTrigger as-child>
        <Button variant="ghost" size="xs" class="text-fg-muted"
          >Session actions</Button
        >
      </PopoverTrigger>
      <PopoverContent
        align="start"
        class="flex w-[min(22rem,72vw)] flex-wrap gap-1.5"
      >
        <Button variant="outline" size="sm" @click="run('/retry')"
          >Retry</Button
        >
        ...
        <Button
          variant="outline"
          size="sm"
          @click="prompt('/title', 'Session title')"
          >Rename</Button
        >
        <Button
          variant="outline"
          size="sm"
          @click="prompt('/branch', 'Branch name (optional)')"
          >Branch</Button
        >
        <Button variant="outline" size="sm" @click="run('/compress --preview')"
