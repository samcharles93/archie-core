...
const used = computed(() => days.value.reduce((a, d) => a + (d.tokens || 0), 0));
const recent = computed(() => days.value.slice(-7).reduce((a, d) => a + (d.tokens || 0), 0));
const perDay = computed(() => (days.value.length ? used.value / days.value.length : 0));
...
<template>
  <Card>
    ...
    <CardContent>
      ...
      <template v-else>
        <div class="mb-1 text-3xl font-semibold tracking-[-0.03em]">{{ compact(projected) }}</div>
        <div class="mb-4 text-xs text-fg-subtle">
          Based on {{ compact(Math.round(perDay)) }} per day over {{ days.length }} days
        </div>
        <SegmentBar
          :segments="[
