...
import { fmtValue, LEVEL_COLOR, levelKind, shortTime, type LogEntry } from "@/lib/log";
...
<template>
  ...
  >
    <span class="font-mono text-fg-subtle">{{ shortTime(props.entry.time) }}</span>
    <span class="text-[10px] font-semibold tracking-[0.04em]" :class="LEVEL_COLOR[levelKind(props.entry.level)]">
      {{ (props.entry.level || "info").toUpperCase() }}
    </span>
    <span class="flex min-w-0 flex-wrap items-baseline gap-2">
      <span class="break-words text-foreground">{{ props.entry.message || props.entry.msg || "" }}</span>
      <span
        v-for="[k, v] in fields()"
        :key="k"
        class="max-w-full truncate rounded-sm bg-muted px-2 text-fg-muted"
      >
        <span class="text-fg-subtle after:opacity-60 after:content-['=']">{{ k }}</span>{{ fmtValue(v) }}
      </span>
    </span>
  </div>
</template>
