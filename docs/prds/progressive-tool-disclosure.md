...
2. **Run the gate in `chatGenerateOptions`** (`internal/app/archied/main.go`)
...
`BuildToolSetFrom` (`internal/agentexec/toolset.go`) documents the invariant:
...
(`telegram_setup.go` build a `TurnRunner` per channel), and the extra
tools (`task_*`, `session_*`, `dashboard_*`) close over that channel's identity
via `TurnRunnerConfig` (`turn.go`). Registering them into `b.toolReg` would
...
(`disclosure.go`), so the extras must be discoverable; the fix is a
...
Replace the current base-set + extra-set merge (`main.go`) with a single
...
`FilterTools` (`disclosure.go`) already returns `all` in full mode and
...
window is resolved in `TurnRunner.prepareTurn` (`turn.go`) but is not
...
(`turn.go`), one production impl (`chat_turn_model.go`), and the test
mock (`turn_test.go`).

### 3. Bridge-handler parity note

`bridgeCallHandler` calls `e.Handler` directly (`disclosure.go`), bypassing
...
`RequiresApproval` tools (`disclosure.go`). This is acceptable for the
...
- Reconcile any `ContextPressureGate` mode with the compression budget already
  computed in `prepareTurn` (`turn.go`).
