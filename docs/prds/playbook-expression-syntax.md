...
This means the lint tool and the daemon share one checker
...
- **J3 -- condition failure semantics.** A condition that errors at
  ...
  gap 1 (`archie-core-t2db.18`) commits
  stop-on-first-failure, so per-action control is not coming and J3
  stands. Alternative considered: cEL separates
...
- **J4 -- `event` stays `dyn` (schema-by-example).** Load-time we
  ...
  schema declarations. Alternative considered: when schema-by-example ships, tighten
...
- **J5 -- cost limit default.** A `CostLimit` at load time (e.g.
  ...
  the same value so they agree. Alternative considered: exact number is a constant to
  tune at first use.
...
gap-2 decision). Conditions make gap 1
...
subsequently solved by that decision: the
...
- Consumption: the playbook event coordinator (open question 4's
  ...
  (`archie-core-t2db.12`) shares the same compile/validate path for author-time
  diagnostics.
