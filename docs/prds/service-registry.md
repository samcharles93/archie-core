...
**Status:** Draft
**Supersedes:** `state-store-contract.md` §10's `State ServiceConnection`
struct-field instruction.
...
services.gatway.target]`), because the struct form leaves it in `toml.MetaData.Undecoded()`.
  ...
  The typo is not silently ignored either way; the behaviour was measured.
