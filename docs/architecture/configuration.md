...
provider settings, and channel settings move to their
respective owners.
...
| ----------------------------------------------------------------------------------------------- | --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `WorkDir`, `StateDir`, `DBPath`                                                                 | root `Config`         | Workspace/storage infrastructure settings, bootstrap-only. `StateDir` holds the embedded NATS store and endpoint file, task logs and the readiness disk target. `DBPath` only locates the legacy SQLite files (`<db_path>-tasks.sqlite`, `-eda.sqlite`, `-conversations.sqlite`) for `archie-state-store import` and the boot gate that refuses to serve until it has run; it goes when the importer does. Defaults derive from the platform data directory. |
...
| forge, dispatch, models, providers, containers, memory, tools and repositories                                 | `IdentityConfig`      | Existing composite duplicates unrelated settings. Replace it with associations from an `IdentityID` to independently owned configured instances.           |
...
| Packaged release data        | `/usr/share/archie/CHANGELOG.md`                                                                                                                      | Packaging/deployment settings or paths derived from an installation prefix.                                                                                                             |
