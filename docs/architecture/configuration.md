...
contract. The core loader MUST NOT gain a field or recognised filename for every
plugin.
...
| ----------------------------------------------------------------------------------------- | --------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `WorkDir`, `StateDir`                                                                     | root `Config`         | Workspace/storage infrastructure settings, bootstrap-only. `StateDir` holds the embedded NATS store and endpoint file, task logs and the readiness disk target. Defaults derive from the platform data directory. |
...
`ControlPlaneService` on the State Store's gRPC server: catalogue, query,
