...
2. **Ansible's `Task`/module system** is a request for a _content_ format:
...
| Content / Projects              | Nearest analogue is the **EDA playbook + module system** (below) plus `internal/skill` (skill catalogue) -- Archie has no "project" = git-checkout-of-automation-content concept the way AWX does                       | `internal/domain/eda/playbook/`, `internal/domain/eda/module/`, `internal/skill/`                                     |
...
| Roles (reusable task bundles)                       | Nearest is `internal/skill` (skill catalogue, frontmatter, tool defs) -- conceptually closer to Claude Skills than Ansible roles                                                                                           | Not a role system; skills are agent-facing capability bundles, not declarative task sequences.                                                                                                                                                                                                                                                                                                                 |
...
4. A much larger Module `Kind` registry, or a deliberate decision to accept
   ...
   fleets of managed hosts). Recommend treating "Ansible-_inspired_ syntax,
   not Ansible-_compatible_ execution" as the working assumption if/when a
