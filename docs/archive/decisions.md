...
The renderer now summarises those legacy events for replay, while current
...
**Current state**: dolt 2.2.2, beads 1.1.0 installed. Beads initialised
...
already exists or the issue is a no-op. The old behaviour parked in an
...
**Reasoning**: Qwythos couldn't complete a representative large-repository
plan stage before timing out. DeepSeek completed it in 21 iterations. Large
repositories need a larger model.
