...
processes on the same host (`carina`): `archie` (bot_user=archie, multiple
repositories, deepseek models) and `winter` (bot_user=winter, a restricted
repository set, local llama.cpp models). Confirmed by diffing
...
7. **Slash-command capabilities** in chat channels (e.g. `/status`, `/spawn`,
   `/approve`) for direct channel interaction.
