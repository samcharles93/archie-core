...
| Engines as ordered preference list | Supports "try Claude first, fall back to Tau, fall back to raw LLM." Each engine declares its own tool surface. |
...
metadata:
  archie:
    ...
    engine: any                      # engine preference (any, claude, tau, raw)
...
| `tau` | Tau (Sam's agent) | Native Go agent with TUI, event bus |
...
arcie-core tries engines in order. If Claude is available and authenticated, it's used. If not, falls to Tau. If Tau isn't built, falls to raw LLM. Each engine declares its budget ceiling.
...
2. **Tau integration depth:** Tau as an engine via CLI, or Tau consuming archie.yaml natively?
3. **Hermes integration:** Should archie-core be a Hermes tool, a Hermes skill, or standalone?
4. **Budget enforcement:** Per-skill budget vs. per-workflow budget vs. monthly cap?
5. **Sandbox image building:** Pre-built images vs. Dockerfile in each skill?
