...
description: >-
  ...
  reviewing behaviour. Load this skill before changing daemon composition,
  ...
  behaviour has one authoritative path. Route proof of duplication, dead-code
...
owner and one production path for each behaviour.

## Read status before code

| Label               | Meaning                                                                     | Authority                                                                                   |
| ------------------- | --------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| **CURRENT**         | Behaviour present in live code and production composition as of 2026-09-18. | Code, tests, and `internal/app/archied` wiring (`cmd/archied/main.go` is a thin dispatcher) |
...
> used from a production entry point. As of 2026-09-18 `internal/domain/`,
> `internal/eventbus/`, `internal/infrastructure/`, and `internal/app/` exist;
> `internal/policy/` does not, and `internal/domain/{agent,identity}` remain
> APPROVED TARGET only.

Resolve apparent disagreement in this order:

1. Read live code and production composition for CURRENT behaviour.
2. Read `docs/prds/01-project-management.md` for APPROVED TARGET ownership.
3. Read the focused decision document for the selected behaviour.
...
Two skills this document routes to are parked outside this checkout, under
`/work/apps/archie-skills-parked/`: `archie-technical-accountability` and
`archie-architecture-planning-campaign`. If your session has not loaded them,
treat each routing target as a manual escalation step rather than a live skill.
Also: `cmd/archied/main.go` is a thin dispatcher; the wiring lives in
...
| Concern              | CURRENT runtime truth on 2026-09-18                                                                                                                                                   | Evidence                                                                                                     |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ |
| Composition          | `cmd/archied/main.go` dispatches to `internal/app/archied.Run()`, which performs construction and wiring in `bootstrap.go`.                                                           | `cmd/archied/main.go`; `internal/app/archied/`                                                               |
| Durable work         | `task.Task` combines forge/chat source data, workflow position, output, retry, identity, and PR state.                                                                                | `internal/domain/workflow/task/task.go`; `internal/domain/storecontract/storecontract.go`                    |
...
| Messaging            | `internal/gateway` contains both the `Message` struct flow and richer `MessageEvent`; channel routing and task control are mixed with conversational behaviour.                       | `internal/gateway/{gateway,messageevent,tasks}.go`; `internal/channels/`                                     |
...
`internal/domain/workflow/task/task.go`, `internal/daemon/daemon.go`, and
`internal/domain/workflow/workflow.go` before changing a transition.
...
part of the wire contract. Earlier non-atomic behaviour is the settled fix,
not an open design.
...
| Generic plugins stay metadata-only              | Keep `plugin.Plugin` at `Name()`/`Version()`. Behaviour on typed capability interface with owning `Registry`/`Manager`.                                 | CURRENT-enforced rule: `internal/plugin/architecture_test.go`                                |
...
| Infrastructure stays replaceable                | Keep forge, persistence, workspace, transport, container, and model details behind behaviour-owned contracts.                                           | APPROVED TARGET: `docs/architecture/dependencies-and-contracts.md`                           |
| Same-repository execution serialises by default | Preserve default unless explicit scheduling policy permits concurrency.                                                                                 | CURRENT: `internal/config/config.go`; `internal/daemon/daemon.go`                            |
...
1. Put cohesive behaviour, vocabulary, state, commands, events, policies,
...
- **Agent** owns persistent assistant, continuity, specialisation, memory.
...
| User-wide               | One user across authorised Agents          |
...
- classify each behavioural value as invariant, runtime setting, policy value,
  derived value, or runtime state.

Source: `docs/architecture/configuration.md`.

## Place new behaviour deliberately

| Behaviour                                                         | Target owner/location                                                                                                                                                                       | Decision gate                                                               |
| ----------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------- |
...
| Persistent assistant behaviour and continuity                     | `internal/domain/agent`                                                                                                                                                                     | Keep model/provider replaceable; do not fold Workflow into Agent.           |
...
| Capability-specific plugin behaviour                              | Owning domain's typed registry/manager                                                                                                                                                      | Never add behaviour to `plugin.Plugin`.                                     |
...
| Deploy assembly                                                   | `deployments/<assembly>`                                                                                                                                                                    | Do not move application behaviour into deployment files.                    |
...
| Weak point on 2026-09-18                                                                                                                                                                                                       | Required response                                                                                         |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------- |
...
| Gateway formerly duplicated lifecycle strings, competing message contracts.                                                                                                                                                    | No third contract or direct lifecycle mutation path; status constants live in `internal/domain/workflow`. |
...
Trace from `cmd/archied/main.go` into `internal/app/archied.Run()`; the wiring
itself is in `internal/app/archied/bootstrap.go`.
