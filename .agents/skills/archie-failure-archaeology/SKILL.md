...
| Tracked `docs-2/node_modules`, 2026-07-28 | Hundreds of pnpm symlink entries become repository content. | **Attempt:** broad `308c199` cleanup/documentation commit. **Cause:** `docs-2/node_modules` was neither ignored nor excluded from the commit. | `git ls-files 'docs-2/node_modules/**'` reports 237 entries at `HEAD`; `.gitignore` has no `node_modules` rule. **Status:** `current/open`. | Track lockfiles, never installed dependency trees. Add the ignore before installation and verify tracked counts are zero. |
...
Re-verify tracked artifact counts: `git ls-files '.gotmp/**' 'docs-2/node_modules/**'`
