...
// TestDaemonRunningVersionsVouchesOnlyForItself: the point of checking a
// pending update report against running versions is that the value checked
// against is ground truth. This process's own compiled-in version is; the
// agent's is not, because runtimeVersion records what archied's release
// pipeline stamped, not what the agent container is actually running.
// Including a guess here would recreate the false success the check exists
// to catch, so its absence is the assertion.
func TestDaemonRunningVersionsVouchesOnlyForItself(t *testing.T) {
	running := daemonRunningVersions()
