...
func TestPluginEngineRuleIsCanonicalAndSharedByAgentInstructions(t *testing.T) {
	...
	architecture := readTestFile(t, filepath.Join(repoRoot, "ARCHITECTURE.md"))
	if !strings.Contains(architecture, engineRuleHeading) {
		t.Fatalf("ARCHITECTURE.md does not contain canonical heading %q", engineRuleHeading)
	}

	claudePath := filepath.Join(repoRoot, "CLAUDE.md")
	agentsPath := filepath.Join(repoRoot, "AGENTS.md")
	claude := readTestFile(t, claudePath)
	if !strings.Contains(claude, engineRuleLink) {
		t.Fatalf("CLAUDE.md does not reference canonical plugin engine rule %q", engineRuleLink)
	}
	...
	if !os.SameFile(claudeInfo, agentsInfo) {
		t.Fatal("AGENTS.md and CLAUDE.md must share one instruction source")
	}
}

func TestGenericPluginContractRemainsMetadataOnly(t *testing.T) {
	t.Parallel()

	pluginType := reflect.TypeFor[plugin.Plugin]()
	got := make([]string, 0, pluginType.NumMethod())
	for method := range pluginType.Methods() {
	...
	if !slices.Equal(got, want) {
		t.Fatalf("plugin.Plugin methods = %v, want metadata-only contract %v; add typed capability family instead", got, want)
	}
}

func TestEveryEngineInterfaceHasTypedCapabilityAndFamilyOwner(t *testing.T) {
	t.Parallel()

	repoRoot := filepath.Clean(filepath.Join("..", ".."))
	families, err := inspectEngineFamilies(filepath.Join(repoRoot, "internal"))
	if err != nil {
		t.Fatal(err)
	}
	if len(families) == 0 {
		t.Fatal("no engine interfaces found; architecture check is not exercising the repository")
	}

	for _, family := range families {
		t.Run(family.packagePath+"."+family.engineName, func(t *testing.T) {
			t.Parallel()
			if !family.hasDomainMethod {
				t.Error("engine exposes only generic metadata; add typed capability methods")
			}
			if !family.hasOwner {
				t.Error("engine has no Registry or Manager family owner")
			}
			if !family.ownerRegisters {
				t.Error("engine family owner does not control provider registration")
			}
			if !family.ownerManages {
				t.Error("engine family owner does not return the registered engine type")
			}
			if !family.singleOwner {
				t.Error("one Registry or Manager must both register and return the engine type")
			}
		})
	}
}
