...
// returns the provider along with the fields that remain.
...
func (g *Gateway) handleModelCommand(
	...
	if refresh {
		g.sendMessage(ctx, b, msg.Chat.ID, msg.MessageThreadID,
			fmt.Sprintf("Active model: %s.", modelDisplayName(snap.ActiveModel, "")))
		return
	}

	// --provider without a specific model: switch provider only.
	if provider != "" && model == "" {
		models := snap.ModelsByProvider[provider]
		if len(models) == 0 {
			g.sendMessage(ctx, b, msg.Chat.ID, msg.MessageThreadID, "Provider switching is not configured.")
			return
		}
		g.switchModel(ctx, b, msg, client, models[0], global)
		return
	}
	...
	// Switch model.
	if model != "" {
		g.switchModel(ctx, b, msg, client, model, global)
		return
	}

	// Neither model nor provider: delegate to inline selector.
	g.sendModelSelector(ctx, b, msg, client)
}
...
func (g *Gateway) switchModel(
	...
	reply, err := client.Route(ctx, messaging.Inbound{
	...
	if strings.HasPrefix(reply.Text, "Cannot switch") {
		g.sendMessage(ctx, b, msg.Chat.ID, msg.MessageThreadID, reply.Text)
		return
	}
	provider, _, _ := strings.Cut(model, "/")
	result := fmt.Sprintf("Model changed to %s.", modelDisplayName(model, provider))
	if global {
...
func (g *Gateway) sendModelSelector(
	...
	params := &bot.SendMessageParams{
		ChatID:      msg.Chat.ID,
		Text:        modelSelectorText(snap.ActiveModel),
		ReplyMarkup: g.modelSelectorKeyboard(snap.ActiveModel, snap.Models, snap.ActiveProvider, snap.ModelsByProvider),
	}
...
func (g *Gateway) modelSelectorKeyboard(active string, models []string, activeProvider string, modelsByProvider map[string][]string) *models.InlineKeyboardMarkup {
	provider := activeProvider
	available := models
	if provider != "" && len(modelsByProvider[provider]) > 0 {
		available = modelsByProvider[provider]
	}
	return g.modelSelectorKeyboardPage(active, available, provider, 0)
}

func (g *Gateway) modelSelectorKeyboardPage(active string, available []string, provider string, page int) *models.InlineKeyboardMarkup {
...
func (g *Gateway) handleModelCallback(
	...
	if strings.HasPrefix(reply.Text, "Cannot switch") {
		g.answerModelCallback(ctx, b, query.ID, reply.Text, true)
		return
	}
	...
	g.editModelPicker(ctx, b, query, modelSelectionConfirmation(client, selected), nil)
}

func modelSelectionConfirmation(manager any, ref string) string {
	...
	detailed, ok := manager.(messaging.DetailedModelManager)
	if !ok {
...
func (g *Gateway) handleModelPageCallback(
	...
	models := snap.ModelsByProvider[page.Provider]
	...
	g.editModelPicker(ctx, b, query,
		modelSelectorTextForProvider(page.Provider, page.Page, len(models)),
		g.modelSelectorKeyboardPage(snap.ActiveModel, models, page.Provider, page.Page))
}
...
func (g *Gateway) handleModelBackCallback(
	...
	g.answerModelCallback(ctx, b, query.ID, "", false)
	g.editModelPicker(ctx, b, query,
		providerSelectorText(snap.ActiveModel, snap.ActiveProvider),
		g.providerSelectorKeyboard(snap.ActiveProvider, snap.Providers, snap.ModelsByProvider))
}
...
func (g *Gateway) handleModelCancelCallback(ctx context.Context, b *bot.Bot, update *models.Update) {
	...
	if _, err := b.EditMessageText(ctx, params); err != nil {
		g.log.Warn("cancel model selector failed", "error", err)
	}
}
...
func (g *Gateway) answerModelCallback(ctx context.Context, b *bot.Bot, queryID, text string, alert bool) {
	...
	if _, err := b.AnswerCallbackQuery(ctx, params); err != nil {
		g.log.Warn("answer model callback query failed", "error", err)
	}
}
...
func (g *Gateway) editModelPicker(
	...
	if _, err := b.EditMessageText(ctx, params); err != nil {
		g.log.Warn("edit model picker failed", "error", err)
	}
}
