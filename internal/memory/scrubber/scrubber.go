...
||||||| parent of 8642f40 (feat: StreamingContextScrubber — strips memory-context fences from LLM output (18.10))
=======
// with no memory blocks). Buffers are pre-allocated and reused. An allocation
// occurs only when a tag spans a chunk boundary (the pending path), which is
// rare in practice.
package scrubber
...
type Scrubber struct {
	...
	// writeErr stores the first write error from the underlying writer.
	// Subsequent writes are skipped once an error is recorded.
	writeErr error
...
func (s *Scrubber) Write(p []byte) (int, error) {
	...
	// Return any stored write error. The full input length is reported as
	// consumed per the io.Writer contract: we always consume the entire
	// input even if the underlying writer fails (the data has been
	// processed; the error is about delivery, not consumption).
...
// writeOutput writes data to the underlying writer. If a previous write
// error has been recorded, writes are silently skipped. New write errors
// are stored for return from Write/Flush.
func (s *Scrubber) writeOutput(data []byte) {
	if len(data) == 0 || s.w == nil || s.writeErr != nil {
		return
	}
...
//
// If the scrubber is inside a memory block when Flush is called, the block
// is implicitly closed (missing closing tag handling). Any pending bytes
// inside the block are stripped, not emitted.
func (s *Scrubber) Flush() error {
	if s.depth > 0 {
...
func (s *Scrubber) Process(chunk string) string {
	...
	prev := s.w
	s.w = &s.buf
	// Restore the original writer even if Write panics.
	defer func() { s.w = prev }()
	_, _ = s.Write([]byte(chunk))
	if s.buf.Len() == 0 {
		return ""
...
// Depth returns the current nesting depth of <memory> blocks.
// The depth is represented as an int; it is the caller's responsibility
// to ensure nesting does not exceed reasonable bounds.
>>>>>>> 8642f40 (feat: StreamingContextScrubber — strips memory-context fences from LLM output (18.10))
