...
func TestGitHubCreateReviewComments(t *testing.T) {
	...
	// The head SHA is deliberately not "headsha": the comments must be pinned
	// to the revision the forge reports for the pull request, not to anything a
	// caller passed in.
	...
	if err := c.CreateReviewComments(t.Context(), "o", "r", 7, comments); err != nil {
		t.Fatal(err)
	}
...
func TestGitHubCreateReviewCommentsKeepsGoingAfterARefusal(t *testing.T) {
	...
	err := c.CreateReviewComments(t.Context(), "o", "r", 7, []InlineReviewComment{
...
func TestGitHubCreateReviewCommentsRefusesAnEmptyHeadSHA(t *testing.T) {
	...
	err := c.CreateReviewComments(t.Context(), "o", "r", 7, []InlineReviewComment{{Path: "a.go", Line: 1, Body: "x"}})
	if err == nil {
...
// caller's. Keeping the interface free of commitID is what lets the anchor be a
// per-forge detail rather than something every caller must resolve.
var _ = func(ctx context.Context, c ReviewCommentWriter) error {
	return c.CreateReviewComments(ctx, "o", "r", 1, nil)
}
